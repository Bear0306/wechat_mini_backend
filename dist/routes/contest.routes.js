...
router.post('/participate', async (req, res) => {
    ...
    if (!notStartedYet) {
        if (now > contest.endAt || contest.status === client_1.ContestStatus.FINALIZED) {
    ...
    // 报名（若已存在则复用）
    const entry = await db_1.prisma.contestEntry.upsert({
        ...
        create: {
            ...
            steps: 0,
            distanceM: 0,
            provider: client_1.DataProvider.OTHER,
            verified: false,
            status: 'PENDING',
            submittedAt: now,
        },
...
router.get('/ended', async (req, res) => {
    ...
    const q = zod_1.z.object({
        ...
        size: zod_1.z.coerce.number().int().min(1).max(50).default(10)
    }).parse(req.query);
    // const uid  = req.user!.uid;
    const uid = 5;
    const today0 = (0, time_1.startOfToday)();
    // participated contest ids
    const participatedIds = (await db_1.prisma.contestEntry.findMany({
    ...
    const itemsAll = await Promise.all(ended.map(async (c) => {
        const my = await db_1.prisma.contestEntry.findFirst({
        ...
        if (my) {
            ...
            const pc = await db_1.prisma.contestPrizeClaim.findFirst({
        ...
        return {
            contestId: c.id,
            title: c.title,
            dateText: (0, time_1.formatCnRange)(c.startAt, c.endAt),
